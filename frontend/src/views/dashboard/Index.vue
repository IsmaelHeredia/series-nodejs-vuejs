...
import { mdiPencil, mdiClose, mdiMagnify } from '@mdi/js';
    ...
    <v-dialog v-model="dialog_preview" max-width="600">
      ...
      <v-card>
        ...
        <v-card-text>
          ...
          <v-img class="align-end text-white" :src="images_url + '/' + imagen" cover height="250px" />

          <h1 class="center preview-titulo">{{ nombre }}</h1>

          <div v-if="links.length > 0">
            <h3 class="center preview-links">Links disponibles</h3>
            <div v-for="link in links" class="center-div">
          ...
          <v-row dense class="center-div preview-calificacion">
            ...
            <v-rating v-model="calificacion" class="rating-form" hover readonly :length="5" :size="32"
        ...
        <v-card-actions class="center-div modal-actions">
          <v-btn class="boton-medio" variant="elevated" color="primary"
            :to="{ name: 'actualizarSerie', params: { id: id } }"><v-icon :icon="mdiPencil" /> Editar</v-btn>
          <v-btn class="boton-medio" variant="elevated" color="primary" @click="cerrarModalPreview"><v-icon
